...
##################################################################################################################
##################################################################################################################
# Below is same script which describe what key and value on what db has been set
##################################################################################################################
##################################################################################################################
#
#
...
#        local value=$(generate_random_string 20)  # 20 character value
#        echo -ne "SET $key $value\r\n"
#        # Print the key, value, and db being set
#        echo "Setting key $key with value $value in db $db on $server:$port"
#      done
#      totalCount=$((totalCount + batchSize))
#    } | redis-cli -c -h "$server" -p "$port" -a "$password" --pipe > /dev/null
#    echo "Sent $totalCount keys to $server:$port so far..."
#  done
#
#  echo "Finished setting $keyCount keys on $server:$port"
#}
